...
void cancel_all_orders() {
    ...
    // Take a snapshot of what needs cancelling BEFORE triggering side effects
    std::vector<OrderId> to_cancel;
    to_cancel.reserve(active_orders_.size());
    for (const auto& order : active_orders_) {
        to_cancel.push_back(order.id);  // Copy stable identifier, NOT iterator/reference
    }
    
    // NOW safe to defer: no iteration, no references to live container
    for (const auto& id : to_cancel) {
        asio::post(strand_, [this, id] {  // Capture by VALUE
            send_cancel(id);
        });
    }
